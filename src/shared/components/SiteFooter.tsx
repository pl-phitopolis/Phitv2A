...
const footerLabelSx = {
  color: "var(--footer-muted)", fontFamily: MONO, fontSize: "0.69rem", fontWeight: 700,
  letterSpacing: "0.12em", textTransform: "uppercase",
} as const;

const contactLinkSx = {
  alignItems: "center", color: "var(--footer-fg)", display: "inline-flex", fontSize: "0.92rem", fontWeight: 600,
  gap: 0.8, textDecoration: "none !important", transition: "color 180ms ease",
  "&:hover, &:focus-visible": { color: NOIR.gold },
} as const;

const legalLinkSx = {
  color: "var(--footer-muted)", fontFamily: MONO, fontSize: "0.7rem", textDecoration: "none !important",
  transition: "color 180ms ease", "&:hover, &:focus-visible": { color: NOIR.gold },
} as const;

export function SiteFooter({ footerAnchorRef, currentNarration, light = false }: SiteFooterProps) {
  ...
  return (
    <Box component="footer" data-home-footer={light || undefined} ref={footerAnchorRef} sx={{ "--footer-fg": light ? NOIR.navyField : NOIR.white, "--footer-muted": light ? NOIR.mist : alpha(NOIR.white, 0.68), "--footer-divider": light ? NOIR.hairline : alpha(NOIR.white, 0.12), bgcolor: light ? NOIR.panel : NOIR.navyDeep, color: "var(--footer-fg)", mt: "auto", overflow: "hidden", position: "relative", py: { xs: 6, md: 8 } }}>
      {!light && <Box aria-hidden sx={{ background: `radial-gradient(circle at 82% 12%, ${alpha(NOIR.gold, 0.12)}, transparent 24%), linear-gradient(145deg, ${NOIR.navyDeep}, ${NOIR.navyFloor})`, inset: 0, pointerEvents: "none", position: "absolute" }} />}
      <Container maxWidth="xl" sx={{ position: "relative", ...(light ? { maxWidth: "1408px !important", px: { xs: "24px", sm: "32px", md: "64px" } } : {}) }}>
        <Grid container columnSpacing={{ xs: 4, md: 6, lg: 10 }} rowSpacing={{ xs: 5, md: 4 }}>
          <Grid size={{ xs: 12, md: 5, lg: light ? 6 : 4 }}>
            <Stack spacing={2.5} sx={{ maxWidth: 410 }}>
              ...
              <Typography sx={{ color: "var(--footer-muted)", fontSize: "0.96rem", lineHeight: 1.65, maxWidth: 360 }}>Research and Development FinTech Firm</Typography>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3, lg: 3 }}>
            <Stack spacing={2}>
              ...
              <Box component="nav" aria-label="Footer navigation" sx={{ display: "grid", gap: 0.35 }}>
                {PRIMARY_LINKS.map((link) => <Box component={RouterLink} key={link.to} to={link.to} underline="none" onClick={handleInternalNavigation(link.to)} sx={{ alignItems: "center", color: "var(--footer-fg)", display: "inline-flex", fontSize: "0.96rem", fontWeight: 600, gap: 0.8, py: 0.45, textDecoration: "none !important", transition: `color 180ms ${EASE_OUT_EXPO_CSS}, transform 180ms ${EASE_OUT_EXPO_CSS}`, width: "fit-content", "&:hover, &:focus-visible": { color: NOIR.gold, transform: "translateX(4px)" } }}>{link.label}<ArrowForwardIcon sx={{ fontSize: 15 }} /></Box>)}
              </Box>
            </Stack>
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            <Stack spacing={2.25}>
              ...
              <Stack spacing={1.65}>
                ...
                <Box sx={{ display: "flex", gap: 1, pt: 0.2 }}><LocationOnIcon sx={{ color: NOIR.gold, fontSize: 18, mt: 0.1 }} /><Typography sx={{ color: "var(--footer-muted)", fontSize: "0.86rem", lineHeight: 1.55 }}>27/F Ecotower, BGC<br />Metro Manila, Philippines</Typography></Box>
              </Stack>
            </Stack>
          </Grid>
          {!light && <Grid size={{ xs: 12, lg: 2 }} sx={{ display: { xs: "none", lg: "block" } }}><Box sx={{ height: "100%", minHeight: 220, opacity: 0.85, position: "relative" }}><LogoParticleField /></Box></Grid>}
        </Grid>
        <Box sx={{ borderTop: "1px solid var(--footer-divider)", mt: { xs: 5, md: 7 }, pt: 2.25 }}>
          <Stack direction={{ xs: "column", md: "row" }} alignItems={{ xs: "flex-start", md: "center" }} justifyContent="space-between" spacing={2}>
            <Stack direction="row" spacing={2.25} sx={{ flexWrap: "wrap" }}>
              <Typography sx={{ color: "var(--footer-muted)", fontFamily: MONO, fontSize: "0.7rem" }}>© {currentYear} Phitopolis International Corp.</Typography>
            ...
            {currentNarration && <Box component={RouterLink} to={currentNarration.next} underline="none" onClick={handleInternalNavigation(currentNarration.next)} sx={{ alignItems: "center", color: "var(--footer-fg)", display: "inline-flex", fontFamily: MONO, fontSize: "0.7rem", fontWeight: 700, gap: 1, letterSpacing: "0.04em", textDecoration: "none !important", transition: `color 180ms ${EASE_OUT_EXPO_CSS}`, "&:hover, &:focus-visible": { color: NOIR.gold } }}>Continue to {currentNarration.label}<ArrowForwardIcon sx={{ fontSize: 16 }} /></Box>}
          </Stack>
        </Box>
      </Container>
    </Box>
